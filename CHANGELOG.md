...
Hand-written. Recorded here because release-please derives its entries from commit subjects,
and a change to the BYTES a generated connector contains is something an existing user needs
told even when no subject line would say so.

**Move these notes down under their version when a release is cut.** release-please inserts
its generated section *below* this one rather than above it, so the move is manual — notes
...
* **spec:** `filter.fields` widens from a flat list of plain-key strings to three entry
  ...
  `f4e9d93d`: of the 40 corpus filter files that hand-write an extractor, **26 can be
  expressed** this way and **14 cannot**; the 14 still emit the stub. `readwise` is one of the
  26 by that measure but is separately rejected for declaring two extractor-taking filters,
  so **25** are generatable today. An earlier note here said **9**, which was a method error —
  it asked whether a file defines a local helper rather than what that helper does; the
  correction and its method are in `docs/ROADMAP.md`'s *Measuring reach*.
  `filter.fields: string[]` and `filter.tags: boolean` keep their exact current meaning and
  byte output; see **Breaking** below for the identifiers now reserved.
...
* **emit(manifest):** a `filesystem` path containing `$&`, `` $` ``, `$'` or `$$` no longer
  ...
  which is new in this same release.
